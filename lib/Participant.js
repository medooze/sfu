...
class Participant
{
	...
	publishStream(streamInfo)
	{
		...
		incomingStream.participantId = this.id;
	...
	addStream(stream, participant) {
		...
		outgoingStream.participantId = participant.getId();
		...
		this.emitter.emit("renegotiationneeded", this.localSDP, this.getOutgoingStreamsMapping());
	...
	getInfo() {
		...
		const info = {
			id	: this.id,
			name	: this.name,
			streams : Array.from(this.incomingStreams.keys())
		};
	...
	getOutgoingStreamsMapping() {
		return Array.from(this.outgoingStreams.values()).map(stream=>[stream.id,stream.participantId]);
	}
