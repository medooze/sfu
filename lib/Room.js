...
class Room
{
	...
	createParticipant(name) 
	{
		...
		participant.on('stream',(stream)=>{
			...
			for (let other of this.participants.values())
				...
				if (participant.getId()!=other.getId())
					...
					other.addStream(stream, participant);
			//Get audio tracks
		...
		participant.on('stopped', () => {
			...
			if (!this.publications.size && !this.participants.size)
			{
				//Log
				this.logger.debug("Starting inactivity timeout");
				//Create a timer for stopping the room
				this.inactivityTimeout = setTimeout(()=>{
					//Log
					this.logger.debug("Room is inactive, stopping it");
					//Stop
					this.stop();
				},10000);
			};
	...
	createPublication()
	{
		this.logger.info("createPublication()");
		...
		//Listener for any new publisher stream
		publication.on('stream',(stream)=>{
			//Send it to the other participants
			for (let other of this.participants.values())
				//Check it is not the event source
				if (publication.getId()!=other.getId())
					//Add stream to participant
					other.addStream(stream, publication);
			//Get video tracks
			const videoTracks = stream.getVideoTracks();
			//For each video track
			for (let i=0; i<videoTracks.length; ++i)
		...
		//Wait for stopped event
		publication.on('stopped', () => {
			//Delete comentarist
			this.publications.delete(publication.id);
			//emir participant change
			this.emitter.emit("publications",this.getInfo().publications);
			//Check if we need to terminate
			if (!this.publications.size && !this.participants.size)
			{
				//Log
	...
	getStreams() {
		...
		for (let participant of this.participants.values())
			...
			for (let stream of participant.getIncomingStreams())
				...
				streams.push([stream,participant]);
		//For each publication
		for (let publication of this.publications.values())
			//For each stream
			for (let stream of publication.getIncomingStreams())
				//Add participant streams
				streams.push([stream,publication]);
		//return them
		return streams;
	}
	...
	getInfo() 
	{
		...
		for (let publication of this.publications.values())
			//Append it
			info.publications.push(publication.getInfo());
